...
/*   Updated: 2021/11/22 12:13:50 by tphlogis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "cub3d.h"
...
void	go_(t_vars *v, int keycode)
{
	...
	if (keycode == KEY_D || keycode == KEY_A)
		si = true;
	if (v->world_map[(int)(v->pozition.x + cf * (v->dir.x * (!si) \
	+ si * sin(v->plane.x)) * v->move_speed)][(int)(v->pozition.y)] \
	== false)
	v->pozition.x += 0.1 * (v->dir.x * (!si) + si * sin(v->plane.x)) * cf;
	if (v->world_map[(int)(v->pozition.x)][(int)(v->pozition.y \
	+ cf * (v->dir.y * (!si) + si * sin(v->plane.y)) * v->move_speed)] \
	== false)
